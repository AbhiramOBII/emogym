    ...
    <form action="{{ route('admin.banners.update', $banner) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="title_kn" id="title_kn" value="{{ old('title_kn', $banner->title_kn) }}"
                       class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-white/50 focus:border-primary focus:ring-1 focus:ring-primary transition-colors">
                @error('title_kn')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-white/50 focus:border-primary focus:ring-1 focus:ring-primary transition-colors">{{ old('description', $banner->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <textarea name="description_kn" id="description_kn" rows="4"
                          class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-white/50 focus:border-primary focus:ring-1 focus:ring-primary transition-colors">{{ old('description_kn', $banner->description_kn) }}</textarea>
        ...
        <!-- Image Upload -->
        <div>
            <label for="image" class="block text-white font-medium mb-2">Banner Image</label>
            <div class="border-2 border-dashed border-white/20 rounded-lg p-6 text-center hover:border-primary/50 transition-colors">
                ...
                <label for="image" class="cursor-pointer">
                    <div id="imagePreview" class="{{ $banner->image ? '' : 'hidden' }} mb-4">
                        <img id="preview" src="{{ $banner->image ? asset('storage/' . $banner->image) : '' }}" alt="Preview" class="max-h-48 mx-auto rounded-lg">
                    </div>
                    <div id="uploadPlaceholder" class="{{ $banner->image ? 'hidden' : '' }}">
                        <i class="fas fa-cloud-upload-alt text-4xl text-white/50 mb-2"></i>
                        <p class="text-white/70">Click to upload new banner image</p>
                        <p class="text-white/50 text-sm">Recommended: 1920x600px, Max 5MB</p>
                    </div>
                    @if($banner->image)
                        <p class="text-white/50 text-sm mt-2">Click to replace current image</p>
                    @endif
                </label>
            </div>
            @error('image')
        ...
        <div>
            ...
            <select name="program_id" id="program_id"
                ...
                @foreach($programs as $program)
                    <option value="{{ $program->id }}" {{ old('program_id', $banner->program_id) == $program->id ? 'selected' : '' }}>{{ $program->title }}</option>
                @endforeach
            </select>
            @error('program_id')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="button_text_kn" id="button_text_kn" value="{{ old('button_text_kn', $banner->button_text_kn) }}"
                       class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-white/50 focus:border-primary focus:ring-1 focus:ring-primary transition-colors">
        ...
        <div>
            ...
            <input type="url" name="button_url" id="button_url" value="{{ old('button_url', $banner->button_url) }}" placeholder="https://example.com"
                   class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-white/50 focus:border-primary focus:ring-1 focus:ring-primary transition-colors">
