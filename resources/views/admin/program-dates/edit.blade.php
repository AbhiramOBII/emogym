...
@section('page-title', 'Edit Program Date')
...
<div class="max-w-6xl">
    ...
    <div class="rounded-lg p-6 shadow-lg" style="background: linear-gradient(155deg, rgba(38, 38, 38, 1) 38%, rgba(42, 42, 42, 1) 100%);">
        <h2 class="text-xl font-bold text-white mb-1">Edit Date</h2>
        <p class="text-white/70 mb-4 text-sm">{{ $program->title }}</p>

        <form action="{{ route('admin.programs.dates.update', [$program, $date]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $date->start_date->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $date->end_date?->format('Y-m-d')) }}"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200">
                    @error('end_date')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $date->start_time ? \Carbon\Carbon::parse($date->start_time)->format('H:i') : '') }}"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200">
                    @error('start_time')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="time" name="end_time" id="end_time" value="{{ old('end_time', $date->end_time ? \Carbon\Carbon::parse($date->end_time)->format('H:i') : '') }}"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200">
                    @error('end_time')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="max_participants" id="max_participants" value="{{ old('max_participants', $date->max_participants) }}" min="1"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                ...
                <div>
                    ...
                    <input type="number" name="current_participants" id="current_participants" value="{{ old('current_participants', $date->current_participants) }}" min="0"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="2"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200">{{ old('notes', $date->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes_kn" id="notes_kn" rows="2"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200">{{ old('notes_kn', $date->notes_kn) }}</textarea>
                    @error('notes_kn')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_available" value="1" {{ old('is_available', $date->is_available) ? 'checked' : '' }}
