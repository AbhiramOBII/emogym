...
<div class="max-w-6xl">
    ...
    <div class="rounded-lg p-6 shadow-lg" style="background: linear-gradient(155deg, rgba(38, 38, 38, 1) 38%, rgba(42, 42, 42, 1) 100%);">
        <h2 class="text-xl font-bold text-white mb-4">Create New Program</h2>
        ...
        <form action="{{ route('admin.programs.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="original_price" id="original_price" value="{{ old('original_price', 0) }}" step="0.01" min="0" required
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                        placeholder="0.00">
                ...
                <div>
                    ...
                    <input type="number" name="discount_percentage" id="discount_percentage" value="{{ old('discount_percentage') }}" step="0.01" min="0" max="100"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                        placeholder="0.00">
                ...
                <div>
                    ...
                    <input type="number" name="discount_amount" id="discount_amount" value="{{ old('discount_amount') }}" step="0.01" min="0"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                        placeholder="0.00">
                ...
                <div>
                    ...
                    <input type="number" name="cost" id="cost" value="{{ old('cost', 0) }}" step="0.01" min="0" required readonly
                ...
                <!-- Image -->
                <div>
                    <label for="image" class="block text-white/70 mb-1 text-sm">Image</label>
                ...
                <div class="md:col-span-3">
                    ...
                    <textarea name="short_description_kn" id="short_description_kn" rows="2"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                        placeholder="ಸಂಕ್ಷಿಪ್ತ ವಿವರಣೆ">{{ old('short_description_kn') }}</textarea>
                ...
                <div class="md:col-span-3">
                    ...
                    <textarea name="who_is_this_for" id="who_is_this_for" rows="2"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                        placeholder="Target audience for this program">{{ old('who_is_this_for') }}</textarea>
                ...
                <div class="md:col-span-3">
                    ...
                    <textarea name="who_is_this_for_kn" id="who_is_this_for_kn" rows="2"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                        placeholder="ಈ ಕಾರ್ಯಕ್ರಮ ಯಾರಿಗೆ?">{{ old('who_is_this_for_kn') }}</textarea>
                ...
                <!-- Link (for online) -->
                <div id="link-field" class="md:col-span-3" style="display: {{ old('type') === 'online' ? 'block' : 'none' }};">
                ...
                <!-- Address (for offline) -->
                <div id="address-field" class="md:col-span-3" style="display: {{ old('type') === 'offline' || !old('type') ? 'block' : 'none' }};">
                    <label for="address" class="block text-white/70 mb-1 text-sm">Address <span class="text-red-500">*</span></label>
                    <textarea name="address" id="address" rows="2"
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                        placeholder="Full address for offline program">{{ old('address') }}</textarea>
                    @error('address')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center md:col-span-2">
                    <label class="flex items-center gap-2 cursor-pointer mt-5">
                        <input type="checkbox" name="is_two_day_experience" value="1" {{ old('is_two_day_experience') ? 'checked' : '' }}
                            class="w-4 h-4 rounded bg-white/10 border-white/20 text-primary focus:ring-primary focus:ring-offset-0">
