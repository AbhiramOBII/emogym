...
<div class="mb-4">
    ...
    <p class="text-white/60 text-sm mt-1">Update service information</p>
</div>
    ...
    <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="title_kn" id="title_kn" value="{{ old('title_kn', $service->title_kn) }}"
                    class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="3" required
                    class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                    placeholder="Enter description in English">{{ old('description', $service->description) }}</textarea>
                @error('description')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description_kn" id="description_kn" rows="3"
                    class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                    placeholder="ವಿವರಣೆ ನಮೂದಿಸಿ">{{ old('description_kn', $service->description_kn) }}</textarea>
            ...
            <!-- Thumbnail Upload -->
            <div class="md:col-span-2">
                <label class="block text-white/70 mb-2 text-sm">Thumbnail Image</label>
                
                <!-- Current Image -->
                @if($service->thumbnail)
                    <div class="mb-3 p-4 bg-white/5 rounded-lg border border-white/10">
                        <p class="text-white/50 text-xs mb-2">Current Image:</p>
                        <img src="{{ asset('storage/' . $service->thumbnail) }}" alt="{{ $service->title }}" class="w-full h-48 object-cover rounded-lg mb-2">
                        <p class="text-white/30 text-xs">Upload a new image to replace</p>
                    </div>
                @endif
                ...
                <div class="mb-3 p-4 bg-white/5 rounded-lg border border-white/10">
                    <div id="imagePreview" class="hidden mb-3">
                        <p class="text-white/50 text-xs mb-2">New Image Preview:</p>
                    ...
                    <label for="thumbnail" class="cursor-pointer">
                        <div id="uploadPlaceholder" class="flex flex-col items-center justify-center py-8 border-2 border-dashed border-white/20 rounded-lg hover:border-primary/50 transition-colors duration-200">
                            ...
                            <p class="text-white/50 text-sm">Click to upload new image</p>
            ...
            <div>
                ...
                <input type="number" name="order" id="order" value="{{ old('order', $service->order) }}" min="0" required
                    class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all duration-200"
                    placeholder="0">
                @error('order')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
